...
def eq_model_trange(t_start, t_stop, major_start_comp, trace_start_comp, P = 1.):
    tstep = 1.
    ...
    for i in xrange(len(trange)):
        if i == 0:
            qa, fa, major_liquid_components, num_iter = state(system_components,trange[-i-1],uaj, ta, P = P)
        else:
            qa, fa, major_liquid_components, num_iter = state(system_components,trange[-i-1],uaj, ta, P = P, fa = fa, liquid_components = major_liquid_components)            
        major_liquid_components['SiO2'] = 1. - sum(major_liquid_components.values())
...
def frac_model_trange(t_start, t_stop, major_start_comp, trace_start_comp, P=1.):
    tstep = 1.
    bulk_d = {key:0 for key in trace_start_comp}
    trange = np.arange(t_stop,t_start, tstep)
    system_components = oxideToComponent(major_start_comp)
    ...
    for i in xrange(len(trange)):
        if i == 0:
            qa, fa, major_liquid_components, num_iter = state(major_liquid_components,trange[-i-1],uaj, ta, P=P)
        else:
            qa, fa, major_liquid_components, num_iter = state(major_liquid_components,trange[-i-1],uaj, ta, P=P,fa = fa, liquid_components = major_liquid_components)
        major_liquid_components['SiO2']=1.-sum(major_liquid_components.values())
        major_oxides = cationFracToWeight(major_liquid_components)
        for key in major_oxides:
            major_oxide_dict[key].append(major_oxides[key])
...
##Equilibrium Crystallization - System Components never Change 
def eq_model_fstop(f_stop, major_start_comp, trace_start_comp, P = 1.):
    tstep = 1.
    bulk_d = {key:0 for key in trace_start_comp}
    system_components = oxideToComponent(major_start_comp)
    ...
    trace_dict = {key:[] for key in trace_start_comp}
    fl = []
    qa, fa,major_liquid_components, num_iter = state(system_components,t,uaj, ta, P = P)
    major_liquid_components['SiO2']=1.-sum(major_liquid_components.values())
    major_oxides = cationFracToWeight(major_liquid_components)
    for key in major_oxides:
        major_oxide_dict[key].append(major_oxides[key])
...
##Fractional Crystallization - System Components change after each iteration
def frac_model_fstop(f_stop, major_start_comp, trace_start_comp, P = 1.): 
    tstep = 1.
    bulk_d = {key:0 for key in trace_start_comp}
    system_components = oxideToComponent(major_start_comp)
    t = get_first_T(system_components)
    ...
    trace_dict = {key:[] for key in trace_start_comp}
    fl = []
    qa, fa, major_liquid_components, num_iter = state(major_liquid_components,t,uaj, ta, P = P)
    major_liquid_components['SiO2']=1.-sum(major_liquid_components.values())
    major_oxides = cationFracToWeight(major_liquid_components)
    for key in major_oxides:
        major_oxide_dict[key].append(major_oxides[key])
